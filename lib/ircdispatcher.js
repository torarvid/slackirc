...
var IrcDispatcher = (function() {
  ...
  var IrcDispatcher = {
    ...
    addServerConfig: function (options) {
      ...
      if (!_.has(serverConfigs, serverKey)) {
        l.verbose('Adding server config for host %s', serverKey);
  ...
  var addClient = function(serverConfig, options, deferred) {
    ...
    var channels = _.values(serverConfig.channelMap);
    return irc.client({
  ...
  var getOrJoinChannel = function(client, options) {
    var ircChannel = client.serverConfig.channelMap[options.channel_id];
    if (!_.contains(client.channels, ircChannel)) {
      l.verbose('Joining channel %s', ircChannel);
      client.join(ircChannel);
      client.channels.push(ircChannel);
    }
    return ircChannel;
  };
