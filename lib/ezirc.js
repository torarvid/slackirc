var EzIrc  = (function() {
  ...
  var EzIrc = {
    connect: function(options) {
      l.verbose('Connecting ' + options.nick + ' to host ' + options.host);
      var deferred = q.defer();
      options.secureProtocol = 'TLSv1_method'; // hack? useful? bad?
      ...
      .then(function(stream) {
        ...
        client.on('nick', function(e) {
          if (client.currentNick === client.me) {
            client.currentNick = e.new;
          }
        });
    ...
    joinIfNecessary: function(client, channel) {
      if (!_.contains(client.joinedChannels, channel)) {
        console.log('Need to join channel to post. Trying to join..');
        console.log(client.joinedChannels, channel);
        return this.join(client, channel);
      }
      console.log('Client is in IRC channel already. Go ahead and post!');
      return client;
    },
  ...
  var setNick = function(client, nick) {
    var desiredNick = client.desiredNick = nick || client.desiredNick;
    if (!client.nickAttempts)
      client.nickAttempts = 0;
    if (client.nickAttempts > 1)
      desiredNick = client.desiredNick + (client.nickAttempts - 1);
    else if (client.nickAttempts == 1)
      desiredNick = client.desiredNick + '_';
    l.info('Trying to change nick from "%s" to "%s"', client.currentNick, desiredNick);
  ...
  var onIrcWelcome = function(e) {
    ...
    l.verbose('Connected user %s with nick %s', client.desiredNick, client.currentNick);
